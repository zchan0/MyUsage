...
struct CostCalculatorTests {
    ...
    // MARK: - Per-bucket cost (spec 16)

    @Test("perBucketCost: Claude buckets line up with the 4-bucket UI")
    func perBucketClaude() {
        let usage = TokenUsage(input: 100_000, output: 20_000, cacheWrite: 50_000, cacheRead: 200_000)
        let buckets = CostCalculator.perBucketCost(
            usage: usage, model: "claude-sonnet-4-5", catalog: catalog()
        )
        #expect(abs(buckets.freshInput    - 0.300) < 1e-6) // 100k * $3
        #expect(abs(buckets.output        - 0.300) < 1e-6) // 20k  * $15
        #expect(abs(buckets.cacheCreation - 0.1875) < 1e-6) // 50k  * $3.75
        #expect(abs(buckets.cacheRead     - 0.060) < 1e-6) // 200k * $0.30
    }

    @Test("perBucketCost.total equals the scalar cost (Claude row)")
    func perBucketTotalMatchesScalarClaude() {
        let usage = TokenUsage(input: 100_000, output: 20_000, cacheWrite: 50_000, cacheRead: 200_000)
        let scalar = CostCalculator.cost(usage: usage, model: "claude-sonnet-4-5", catalog: catalog())
        let buckets = CostCalculator.perBucketCost(usage: usage, model: "claude-sonnet-4-5", catalog: catalog())
        #expect(abs(buckets.total - scalar) < 1e-9)
    }

    @Test("perBucketCost.total equals the scalar cost (Codex row)")
    func perBucketTotalMatchesScalarCodex() {
        let usage = TokenUsage(input: 1_000_000, output: 100_000, cachedInput: 500_000)
        let scalar = CostCalculator.cost(usage: usage, model: "gpt-5-codex", catalog: catalog())
        let buckets = CostCalculator.perBucketCost(usage: usage, model: "gpt-5-codex", catalog: catalog())
        #expect(abs(buckets.total - scalar) < 1e-9)
    }

    @Test("perBucketTotalCost sum equals scalar totalCost over mixed providers")
    func perBucketTotalSumMatches() {
        let byModel: UsageByModel = [
            "claude-sonnet-4-5": TokenUsage(input: 100_000, output: 20_000, cacheWrite: 50_000, cacheRead: 200_000),
            "gpt-5-codex":       TokenUsage(input: 1_000_000, output: 100_000, cachedInput: 250_000)
        ]
        let scalar = CostCalculator.totalCost(of: byModel, catalog: catalog())
        let buckets = CostCalculator.perBucketTotalCost(of: byModel, catalog: catalog())
        #expect(abs(buckets.total - scalar) < 1e-9)
    }

    @Test("perBucketCost on unknown model returns zero")
    func perBucketUnknownZero() {
        let buckets = CostCalculator.perBucketCost(
            usage: TokenUsage(input: 100_000, output: 100_000),
            model: "no-such-model",
            catalog: catalog()
        )
        #expect(buckets == .zero)
