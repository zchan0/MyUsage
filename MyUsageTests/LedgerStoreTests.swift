...
struct LedgerStoreTests {
    ...
    @Test("deleteRows(forDevice:provider:accountID:) wipes only the matching slice")
    func deleteRowsForAccount() throws {
        let store = try makeStore()
        let now = Date(timeIntervalSince1970: 1_000_000)
        _ = try store.upsert([
            // Mac1, account A — TARGETS for delete
            LedgerEntry(deviceId: "Mac1", accountId: "a@x.com", provider: .claude, day: "2026-04-01", costUSD: 5, recordedAt: now),
            LedgerEntry(deviceId: "Mac1", accountId: "a@x.com", provider: .claude, day: "2026-04-02", costUSD: 7, recordedAt: now),
            // Mac1, account B — different account, must stay
            LedgerEntry(deviceId: "Mac1", accountId: "b@x.com", provider: .claude, day: "2026-04-01", costUSD: 1, recordedAt: now),
            // Mac2, account A — different device, must stay (peer's truth)
            LedgerEntry(deviceId: "Mac2", accountId: "a@x.com", provider: .claude, day: "2026-04-01", costUSD: 99, recordedAt: now),
            // Mac1, account A, codex — different provider, must stay
            LedgerEntry(deviceId: "Mac1", accountId: "a@x.com", provider: .codex, day: "2026-04-01", costUSD: 100, recordedAt: now)
        ])

        let deleted = try store.deleteRows(forDevice: "Mac1", provider: .claude, accountID: "a@x.com")
        #expect(deleted == 2)

        // Cross-device totals: Mac1's contribution gone, Mac2's intact.
        let totals = try store.monthlyTotalsByAccount(provider: .claude, monthKey: "2026-04")
        #expect(totals["a@x.com"] == 99, "only Mac2's row remains")
        #expect(totals["b@x.com"] == 1)

        // Codex untouched.
        let codex = try store.monthlyTotalsByAccount(provider: .codex, monthKey: "2026-04")
        #expect(codex["a@x.com"] == 100)
    }

    @Test("deleteRows(forDevice:provider:accountID:) is a no-op when nothing matches")
    func deleteRowsForAccountNoop() throws {
        let store = try makeStore()
        let now = Date(timeIntervalSince1970: 1_000_000)
        _ = try store.upsert([
            LedgerEntry(deviceId: "Mac1", accountId: "a@x.com", provider: .claude, day: "2026-04-01", costUSD: 5, recordedAt: now)
        ])
        let deleted = try store.deleteRows(forDevice: "Mac1", provider: .claude, accountID: "never@x.com")
        #expect(deleted == 0)
        let totals = try store.monthlyTotalsByAccount(provider: .claude, monthKey: "2026-04")
        #expect(totals["a@x.com"] == 5)
    }
